...
from agente_ia.schemas import Briefing, EvaluatedArticle

PRIMARY_COLOR = "#5B2A86"
ACCENT_COLOR = "#00B3A4"
SOFT_BG = "#F5F3FB"
TEXT_COLOR = "#1F2937"
MUTED_COLOR = "#6B7280"
...
<body style="margin:0;padding:0;background:#E7E5EE;font-family:Segoe UI,Arial,sans-serif;color:{{ text }};">
<table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0" bgcolor="#E7E5EE">
  <tr><td align="center" style="padding:24px 12px;">
    ...
    <table role="presentation" width="640" cellpadding="0" cellspacing="0" border="0"
      ...
      <tr>
        <td bgcolor="{{ primary }}" style="padding:24px 28px;color:#FFFFFF;">
          <table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0">
            <tr>
              <td style="font-size:12px;letter-spacing:2px;color:#D7C9EC;text-transform:uppercase;">
                Briefing IA - AI Engineer
              </td>
              <td align="right" style="font-size:12px;color:#D7C9EC;">{{ today }}</td>
            </tr>
            <tr>
      ...
      <!-- skill del dia -->
      {% if briefing.skill_of_the_day %}
      <tr>
        <td style="padding:6px 28px 8px 28px;">
          <table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0"
                 style="background:#FFF7E6;border-left:3px solid #F59E0B;border-radius:4px;">
            <tr>
              <td style="padding:12px 16px;font-size:14px;line-height:1.5;color:{{ text }};">
                <strong style="color:#B45309;">Skill para hoy:</strong>
                {{ briefing.skill_of_the_day }}
              </td>
            </tr>
          </table>
        </td>
      ...
      <tr>
        <td style="padding:14px 28px 0 28px;">
          <table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0"
                 style="border:1px solid #E5E1F0;border-radius:6px;">
            <tr>
                ...
                <table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0"
                  ...
                  <tr>
                    <td style="padding:10px 12px;font-size:13px;line-height:1.5;color:{{ text }};">
                      <strong style="color:{{ primary }};">Por que importa:</strong>
                      {{ item.evaluation.why_relevant }}<br>
                      <strong style="color:{{ primary }};">Que aprender:</strong>
                      {{ item.evaluation.what_to_learn }}
                    </td>
                  </tr>
                </table>
              </td>
                ...
                <table role="presentation" cellpadding="0" cellspacing="0" border="0">
                  <tr>
                    ...
                    <td width="14" height="10"
                        bgcolor="{% if n <= item.evaluation.relevance_score %}{{ accent }}{% else %}#E0DCEA{% endif %}"
      ...
      <tr>
        <td style="padding:24px 28px 28px 28px;">
          <hr style="border:0;border-top:1px solid #E5E1F0;margin:0 0 12px 0;">
          <div style="font-size:12px;color:{{ muted }};line-height:1.5;">
            Generado automaticamente por <strong>Agente IA Personal</strong>
...
def _group_by_category(items: Iterable[EvaluatedArticle]) -> list[tuple[str, list[EvaluatedArticle]]]:
    """Agrupa articulos por su categoria."""
    grouped: OrderedDict[str, list[EvaluatedArticle]] = OrderedDict()
    for it in items:
        area = it.evaluation.category or "Industria y carrera"
